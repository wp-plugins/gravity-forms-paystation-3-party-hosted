...
class GFPaystationPlugin {
	...
	*/
	...
	public function gformConfirmation($confirmation, $form, $entry, $ajax) {
		...
		$paymentReq->merchantReference  = $formData->MerchantReference;         // Set other things.
                ...
                if ((isset($params[GFPAYSTATION_POSTBACK])) && ($params[GFPAYSTATION_POSTBACK] == $pluginOptions['securityHash']))
                {
                    try {
                        ...
                        if ($resultReq->isValid) {
                            ...
                            // The home page of the site is displayed so that will satisfy the paystation system that the response was recieved.
                        }
                    }
	...
	*/
	...
	protected function hasFormBeenProcessed($form) {
		...
		$sql = "select lead_id from {$wpdb->prefix}rg_lead_meta where meta_key='gfpaystation_unique_id' and meta_value = %s";
